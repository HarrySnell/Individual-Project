...
int main() {
	...
	const cl_float3 rotationAxisP1 = { 1,1,1 }; // Defines the axis origin
	const cl_float3 rotationAxisP2 = { 5,2,3 }; // Defines a point on the axis
	const float theta = 0.001; // Defines the rotation angle per frame
	///////////////////////////////////////////////////////////////
	...
	std::ifstream testFile("GA_RT_Y_AXIS_ROTATION_kernel.cl"); // KERNEL NAME HERE !!!!
	...
	// Creates the axis of rotation 
	float rotationAxisP1Conformal[5];
	rotationAxisP1Conformal[0] = rotationAxisP1.x;
	rotationAxisP1Conformal[1] = rotationAxisP1.y;
	rotationAxisP1Conformal[2] = rotationAxisP1.z;
	rotationAxisP1Conformal[3] = ((rotationAxisP1.x * rotationAxisP1.x) + (rotationAxisP1.y * rotationAxisP1.y) + (rotationAxisP1.z * rotationAxisP1.z)) / 2.0;
	rotationAxisP1Conformal[4] = 1.0;
	float rotationAxisP2Conformal[5];
	rotationAxisP2Conformal[0] = rotationAxisP2.x;
	rotationAxisP2Conformal[1] = rotationAxisP2.y;
	rotationAxisP2Conformal[2] = rotationAxisP2.z;
	rotationAxisP2Conformal[3] = ((rotationAxisP2.x * rotationAxisP2.x) + (rotationAxisP2.y * rotationAxisP2.y) + (rotationAxisP2.z * rotationAxisP2.z)) / 2.0;
	rotationAxisP2Conformal[4] = 1.0;
	float rotationAxis[6];
	rotationAxis[0] = rotationAxisP1Conformal[2] + (-rotationAxisP2Conformal[2]); // e1 ^ e2
	rotationAxis[1] = (-(rotationAxisP1Conformal[1] + (-rotationAxisP2Conformal[1]))); // e1 ^ e3
	rotationAxis[2] = (-(rotationAxisP1Conformal[1] * rotationAxisP2Conformal[2] + (-(rotationAxisP1Conformal[2] * rotationAxisP2Conformal[1])))); // e1 ^ einf
	rotationAxis[3] = rotationAxisP1Conformal[0] + (-rotationAxisP2Conformal[0]); // e2 ^ e3
	rotationAxis[4] = rotationAxisP1Conformal[0] * rotationAxisP2Conformal[2] + (-(rotationAxisP1Conformal[2] * rotationAxisP2Conformal[0])); // e2 ^ einf
	rotationAxis[5] = (-(rotationAxisP1Conformal[0] * rotationAxisP2Conformal[1] + (-(rotationAxisP1Conformal[1] * rotationAxisP2Conformal[0])))); // e3 ^ einf
	// Creates the rotation operator (R) and it s reverse (R_reverse)
	float R[14];
	R[0] = cos(theta / 2.0); // 1.0
	R[6] = (-(rotationAxis[0] * sin(theta / 2.0))); // e1 ^ e2
	R[7] = (-(rotationAxis[1] * sin(theta / 2.0))); // e1 ^ e3
	R[8] = (-(rotationAxis[2] * sin(theta / 2.0))); // e1 ^ einf
	R[10] = (-(rotationAxis[3] * sin(theta / 2.0))); // e2 ^ e3
	R[11] = (-(rotationAxis[4] * sin(theta / 2.0))); // e2 ^ einf
	R[13] = (-(rotationAxis[5] * sin(theta / 2.0))); // e3 ^ einf
	float R_reverse[14];
	R_reverse[0] = R[0]; 
	R_reverse[6] = -R[6];
	R_reverse[7] = -R[7];
	...
	for (int f = 0; f < frames; f++) {
		...
		///////////////////////////////////////////////////////////////
		if (f != 0) {
			// Rotates the scene spheres about the axis
			for (int n = 0; n < numberOfSpheres; n++) {
				float spheres_[5];
				spheres_[0] = (R[0] * spheres[n].s0 + R[6] * spheres[n].s1 + R[7] * spheres[n].s2 + (-R[8])) * R_reverse[0] + (-((R[0] * spheres[n].s1 + (-(R[6] * spheres[n].s0)) + R[10] * spheres[n].s2 + (-R[11])) * R_reverse[6])) + (-((R[0] * spheres[n].s2 + (-(R[7] * spheres[n].s0)) + (-(R[10] * spheres[n].s1)) + (-R[13])) * R_reverse[7])) + R[0] * R_reverse[8] + (-((R[6] * spheres[n].s2 + (-(R[7] * spheres[n].s1)) + R[10] * spheres[n].s0) * R_reverse[10])) + R[6] * R_reverse[11] + R[7] * R_reverse[13]; // e1
				spheres_[1] = (R[0] * spheres[n].s0 + R[6] * spheres[n].s1 + R[7] * spheres[n].s2 + (-R[8])) * R_reverse[6] + (R[0] * spheres[n].s1 + (-(R[6] * spheres[n].s0)) + R[10] * spheres[n].s2 + (-R[11])) * R_reverse[0] + (-((R[0] * spheres[n].s2 + (-(R[7] * spheres[n].s0)) + (-(R[10] * spheres[n].s1)) + (-R[13])) * R_reverse[10])) + R[0] * R_reverse[11] + (R[6] * spheres[n].s2 + (-(R[7] * spheres[n].s1)) + R[10] * spheres[n].s0) * R_reverse[7] + (-(R[6] * R_reverse[8])) + R[10] * R_reverse[13]; // e2
				spheres_[2] = (R[0] * spheres[n].s0 + R[6] * spheres[n].s1 + R[7] * spheres[n].s2 + (-R[8])) * R_reverse[7] + (R[0] * spheres[n].s1 + (-(R[6] * spheres[n].s0)) + R[10] * spheres[n].s2 + (-R[11])) * R_reverse[10] + (R[0] * spheres[n].s2 + (-(R[7] * spheres[n].s0)) + (-(R[10] * spheres[n].s1)) + (-R[13])) * R_reverse[0] + R[0] * R_reverse[13] + (-((R[6] * spheres[n].s2 + (-(R[7] * spheres[n].s1)) + R[10] * spheres[n].s0) * R_reverse[6])) + (-(R[7] * R_reverse[8])) + (-(R[10] * R_reverse[11])); // e3
				spheres_[3] = (R[0] * spheres[n].s0 + R[6] * spheres[n].s1 + R[7] * spheres[n].s2 + (-R[8])) * R_reverse[8] + (R[0] * spheres[n].s1 + (-(R[6] * spheres[n].s0)) + R[10] * spheres[n].s2 + (-R[11])) * R_reverse[11] + (R[0] * spheres[n].s2 + (-(R[7] * spheres[n].s0)) + (-(R[10] * spheres[n].s1)) + (-R[13])) * R_reverse[13] + (R[0] * spheres[n].s3 + (-(R[8] * spheres[n].s0)) + (-(R[11] * spheres[n].s1)) + (-(R[13] * spheres[n].s2))) * R_reverse[0] + (-((R[6] * spheres[n].s3 + (-(R[8] * spheres[n].s1)) + R[11] * spheres[n].s0) * R_reverse[6])) + (-((R[7] * spheres[n].s3 + (-(R[8] * spheres[n].s2)) + R[13] * spheres[n].s0) * R_reverse[7])) + (-(R[8] * R_reverse[8])) + (-((R[10] * spheres[n].s3 + (-(R[11] * spheres[n].s2)) + R[13] * spheres[n].s1) * R_reverse[10])) + (-(R[11] * R_reverse[11])) + (-(R[13] * R_reverse[13])); // einf
				spheres_[4] = R[0] * R_reverse[0] + (-(R[6] * R_reverse[6])) + (-(R[7] * R_reverse[7])) + (-(R[10] * R_reverse[10])); // e0
				spheres[n].s0 = spheres_[0];
				spheres[n].s1 = spheres_[1];
				spheres[n].s2 = spheres_[2];
				spheres[n].s3 = spheres_[3];
				spheres[n].s4 = spheres_[4];
			}
		}
		///////////////////////////////////////////////////////////////
