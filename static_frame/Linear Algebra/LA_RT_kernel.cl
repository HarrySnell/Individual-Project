...
int numberOfSpheres
		...
		if(planeLambda == INFINITY && sphereLambda == INFINITY){
			...
			// Creates a line from the closest primary intersection to the light source (the shadow ray)
			// 
			...
			if(shadowFlag == 1){
				...
				if(sphereFlag == 1){
					// Calculates the angle between the shadow ray and the sphere normal  
