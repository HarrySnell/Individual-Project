...
// Date: 30/04/17
// Desciption: OpenCL linear algebra ray tracer with translation - OpenCL kernel  
// File 2 of 2 
///////////////////////////////////////////////////////////
// N.B. As this file is almost identical to the kernel for the linear algebra static geomrtry ray tracer, 
...
__kernel void testKernel(
	...
	// Does the primary ray intersect the plane?
			...
			if(shadowRaySphereDiscriminate >= 0){
				if(sphereFlag == 0){
					...
					if( t != closestSphereIndex){shadowFlag = 1;}
						...
						// current sphere is the prip sphere 
						// how do we determine that the srip that is not the prip is closer to the ls than the prip? 
						if(shadowRaySphereDiscriminate > 0){
							//float prip_distanceToLightSource = shadowRayMagnitude
							// find both srips
							// discard the srip = prip 
							// calculate distance from srip_ to ls 
