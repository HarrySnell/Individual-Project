...
int main() {
	...
	const cl_float theta = 0.001; // Defines the angle of rotation per frame
	///////////////////////////////////////////////////////////
	...
	std::ifstream testFile("LA_RT_Y_AXIS_ROTATION_kernel.cl"); 
	...
	for (int f = 0; f < frames; f++) {
		...
		if (f != 0) {
			// Rotates each of the spheres 
			for (int n = 0; n < numberOfSpheres; n++) {
				cl_float3 spheres_;
				spheres_.s0 = (spheres[n].s2 * sin(theta)) + (spheres[n].s0 * cos(theta));
				spheres_.s2 = (spheres[n].s2 * cos(theta)) - (spheres[n].s0 * sin(theta));
				spheres[n].s0 = spheres_.s0;
				spheres[n].s2 = spheres_.s2;
			}
			// Rotates the light position, and the plane. 
			cl_float3 lightPosition_;
			lightPosition_.x = (lightPosition.z * sin(theta)) + (lightPosition.x * cos(theta));
			lightPosition_.z = (lightPosition.z * cos(theta)) - (lightPosition.x * sin(theta));
			lightPosition.x = lightPosition_.x;
			lightPosition.z = lightPosition_.z;
			cl_float3 planePoint_;
			planePoint_.x = (planePoint.z * sin(theta)) + (planePoint.x * cos(theta));
			planePoint_.z = (planePoint.z * cos(theta)) - (planePoint.x * sin(theta));
			planePoint.x = planePoint_.x;
			planePoint.z = planePoint_.z;
		}
		///////////////////////////////////////////////////////////
